...
function fm(v,s){return s+Math.round(v).toLocaleString('fr-FR');}
...
function renderResults(d){
  ...
  g('h-before').textContent=fv(d.tmy)+'/an';
  g('h-after').textContent=fv(d.tmty)+'/an';
  ...
  g('br-before').textContent=fv(d.cbm*12)+'/an';
  g('br-before-d').textContent=`${fv(d.cbp)}/incident · ${fv(d.cbm)}/mois`;
  g('br-after').textContent=fv(d.cbmt*12)+'/an';
  g('br-after-d').textContent=`${fv(d.cbpr)}/incident en recette · ${fv(d.cbmt)}/mois`;
  ...
  g('t-cn').textContent=Math.round(d.acno)+' cas/mois';
  g('t-ct').textContent=Math.round(d.acno*3)+' cas/mois';
...
function live(){
  const d=cAll();
  g('exb').textContent=`Exécuteur principal : ${d.ex.label} · ${d.s}${Math.round(d.ex.rate)}/h`;
...
function renderDetailedReport(d){
  ...
  const brs=[
    ['Triage et reproduction','Dev','1,5h',1.5*r.DEV],
    ['Développement du correctif','Dev','5h',5*r.DEV],
    ['Revue et validation (senior)','Dev','1,5h',1.5*r.DEV],
    ['Retest et redéploiement','Dev','1,5h',1.5*r.DEV],
    ['Overhead coordination urgence','Dev','+50%',TP*r.DEV*0.5],
    ['Retest régression',d.ex.label+'/QA','1,5h',1.5*d.ex.rate],
    ['Communication parties prenantes','PO','0,5h',0.5*r.PO],
    ['Interruption équipe',`${d.dv} dev`,`${d.dv*0.5}h`,d.intr],
    ['TOTAL PAR INCIDENT EN PRODUCTION','','',d.cbp],
    ['Incident intercepté en recette','Dev+QA','2,5h',d.cbpr],
    ['ÉCONOMIE NETTE PAR INCIDENT','','',d.cbp-d.cbpr]
  ];
  ...
  g('d4-gain').textContent='+ '+fv(d.bsm)+'/mois';

  g('syn-big').textContent=fv(d.sy);
  g('x-cost-b').textContent=fv(d.tmy);
  g('x-cost-a').textContent=fv(d.tmty);
  g('x-time-b').innerHTML=(d.m80n<500?Math.ceil(d.m80n):'∞')+'<small> mois</small>';
  g('x-time-a').innerHTML=(d.m80t<500?Math.ceil(d.m80t):'∞')+'<small> mois</small>';
  g('x-bug-b').innerHTML=Math.round(d.bg*12)+'<small> /an</small>';
  g('x-bug-a').innerHTML=Math.round(d.bg*12-d.bprev*12)+'<small> /an</small>';
  g('x-dev-b').textContent=fv(d.cbp);
  g('x-dev-a').textContent=fv(d.cbpr);

  const asm=[
    ['Accélération tests manuels','10× plus rapide (parallélisation + génération no-code)'],
    ['Accélération automatisation','3× plus rapide vs Playwright / Selenium'],
    ['Réduction incidents en production','30% interceptés avant mise en ligne'],
    ['Temps de traitement en production',`${TP}h dev + overhead ×${OVH}`],
    ['Temps de traitement en recette',`${TPR}h dev + QA 0,5h`],
    ['Salaire QA','EU 60k€ / US 120k$'],
    ['Salaire Dev','EU 80k€ / US 140k$'],
    ['Salaire PO / BA','EU 85k€ / US 175k$']
  ];
...
const CDD_META={
  'cdd-region':{
    hiddenId:'region',
    getDisplay:(val)=>{
      const map={EU:'Europe',US:'États-Unis'};
      return`<div class="cdd-label">${map[val]}</div>`;
    }
  },
  'cdd-rel':{
    hiddenId:'s-rel',
    getDisplay:(val)=>`<div class="cdd-label">${val} par mois</div>`
  }
};
...
function initHubSpotForm(){
  if(typeof hbspt !== 'undefined' && hbspt.forms){
    hbspt.forms.create({
      ...
      translations: {
        fr: {
          submitText: "Débloquer le rapport →"
        }
      },
